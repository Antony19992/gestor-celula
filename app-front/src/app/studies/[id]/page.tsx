...
export default function StudyPage() {
  ...
  useEffect(() => {
    // Tenta servir do cache da lista antes de ir ao servidor
  ...
  if (error || !study) {
    return (
      <div className="space-y-4">
        <Link href="/studies" className="inline-flex items-center gap-1 text-sm text-gray-500 hover:text-gray-700">
          ← Voltar
        </Link>
        <Card><p className="text-sm text-red-600">{error ?? "Estudo não encontrado."}</p></Card>
      </div>
    );
  }
    ...
    <div className="space-y-4">
      ...
      <div className="flex items-center gap-3">
        ...
        <div className="min-w-0 flex-1">
          <h1 className="truncate text-xl font-bold text-gray-900">{study.title}</h1>
          <p className="text-xs italic text-gray-500">{study.verse}</p>
        </div>
        <button
            ...
            questions.map((text, idx) => (
              <Card key={idx}>
                <div className="flex items-start gap-3">
                  ...
                  <p className={`flex-1 pt-0.5 leading-relaxed text-gray-800 ${fontSize === "sm" ? "text-sm" : fontSize === "base" ? "text-lg" : "text-2xl"}`}>
                    {text}
                  </p>
                </div>
              </Card>
