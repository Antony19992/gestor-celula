...
export default function BibliaPage() {
  ...
  const fontLabel = fontSize === "sm" ? "Aa" : fontSize === "base" ? "Aa+" : "Aa++";
  const maxChapter = bookMeta?.chapterCount ?? 1;

  return (
    <>
      <div className="sticky top-14 z-30 -mx-4 flex items-center gap-2 border-b border-gray-100 bg-white px-4 py-2.5 dark:border-gray-800 dark:bg-gray-900">
        <button onClick={openModal} className="flex min-w-0 flex-1 items-center gap-1 text-left">
        ...
        <button onClick={() => setChapter((c) => Math.max(0, c - 1))} disabled={chapter === 0}
          className="flex h-8 w-8 shrink-0 items-center justify-center rounded-lg border border-gray-200 text-xl text-gray-500 transition-colors hover:bg-gray-50 disabled:opacity-30 dark:border-gray-700 dark:text-gray-400 dark:hover:bg-gray-800">
          ‹
        </button>
        <button onClick={() => setChapter((c) => Math.min(maxChapter - 1, c + 1))} disabled={chapter >= maxChapter - 1}
          className="flex h-8 w-8 shrink-0 items-center justify-center rounded-lg border border-gray-200 text-xl text-gray-500 transition-colors hover:bg-gray-50 disabled:opacity-30 dark:border-gray-700 dark:text-gray-400 dark:hover:bg-gray-800">
          ›
        </button>
      </div>
      ...
      <div className="mt-4 pb-8">
        ...
        {!loading && !error && (
          <div className={`space-y-3 ${fontClass}`}>
            ...
            {verses.length === 0 && (
              <p className="py-8 text-center text-sm text-gray-400 dark:text-gray-500">Nenhum versículo encontrado.</p>
            )}
          </div>
        )}
      </div>
      ...
      {showModal && (
        <div className="fixed inset-0 z-50 flex flex-col justify-end bg-black/50 backdrop-blur-sm" onClick={() => setShowModal(false)}>
          <div className="max-h-[78vh] overflow-hidden rounded-t-2xl bg-white dark:bg-gray-900" onClick={(e) => e.stopPropagation()}>
            {step === "book" ? (
              <BookSelector testament={testament} onTestamentChange={setTestament} currentAbbrev={abbrev} onSelect={handleBookSelect} />
            ) : (
              <ChapterSelector bookMeta={pendingMeta} currentChapter={pendingAbbrev === abbrev ? chapter : -1} onSelect={handleChapterSelect} onBack={() => setStep("book")} />
            )}
          </div>
        </div>
      )}
...
function BookSelector({
  testament, onTestamentChange, currentAbbrev, onSelect,
}: {
  testament: "AT" | "NT"; onTestamentChange: (t: "AT" | "NT") => void;
  currentAbbrev: string; onSelect: (abbrev: string) => void;
}) {
    ...
    <div className="flex h-full flex-col">
      <div className="flex shrink-0 border-b border-gray-100 dark:border-gray-800">
        {(["AT", "NT"] as const).map((t) => (
          <button key={t} onClick={() => onTestamentChange(t)}
